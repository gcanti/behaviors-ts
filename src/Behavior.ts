...
export function map<A, B>(f: (a: A) => B, fa: Behavior<A>): Behavior<B> {
  return fa.map(f)
}

export function of<A>(a: A): Behavior<A> {
  return new Behavior<A>(<B>(e: Event<(a: A) => B>) => e.map(f => f(a)))
}

export function ap<A, B>(fab: Behavior<(a: A) => B>, fa: Behavior<A>): Behavior<B> {
  return fa.ap(fab)
}
...
export function step<A>(a: A, fa: Event<A>): Behavior<A> {
  return new Behavior<A>(<B>(e: Event<(a: A) => B>) => event.sampleOn(event.of(a).alt(fa), e))
}
 ...
 */
export function unfold<A, B>(f: (a: A, b: B) => B, fa: Event<A>, b: B): Behavior<B> {
  return step<B>(b, event.fold(f, fa, b))
}
...
export function sampleBy<A, B, C>(f: (a: A) => (b: B) => C, ba: Behavior<A>, eb: Event<B>): Event<C> {
...
export function sample_<A, B>(ba: Behavior<A>, eb: Event<B>): Event<A> {
  return sampleBy(constant, ba, eb)
}
...
export function integral<T, A>(
  ...
  return new Behavior<A>(<B>(e: Event<(a: A) => B>) => {
    ...
    const y = event.withLast(sampleBy<T, A, [T, A]>(t => a => [t, a], bt, x))
    const z = event.fold(approx, y, initial)
    return z.ap(e)
  })
}
...
export function fixB<A>(a: A, f: (ba: Behavior<A>) => Behavior<A>): Behavior<A> {
  return new Behavior<A>(<B>(s: Event<(a: A) => B>) => {
    const { event: evt, push } = event.create<A>().run()
    const b = f(step(a, evt))
    sample_(b, s).subscribe(a => push(a).run())
    return event.sampleOn(evt, s)
  })
}
...
export function animate<S>(scene: Behavior<S>, render: (s: S) => IO<void>): IO<void> {
  return event.subscribe(sample_(scene, animationFrame()), render)
}
